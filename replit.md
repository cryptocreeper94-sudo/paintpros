...
The platform utilizes a multi-tenant architecture managed via `client/src/config/tenant.ts`. Each tenant can customize branding, services, pricing, SEO metadata, feature toggles, and credentials. Tenant selection is driven by `VITE_TENANT_ID`.
