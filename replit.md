...
- **Color Library System:** Curated professional paint color database featuring Sherwin-Williams and Benjamin Moore colors. Features include horizontal scrolling glassmorphic cards, flip cards with color details (LRV, undertone, coordinating colors), brand and category filtering, search functionality, and integration with the estimate flow. Database tables: `paintColors` (brand, productLine, colorCode, colorName, hexValue, category, undertone, lrv, coordinatingColors, trimColors, roomTypes), `customerColorSelections` (for saving customer color choices per estimate). Key component: `client/src/pages/color-library.tsx`.
