...
export function LeadCaptureModal({ tenantId = "paintprosco", tenantName = "Paint Pros Co" }: LeadCaptureModalProps) {
  ...
  const brandColor = isLume ? "#1e3a5f" : "#1e3a5f"; // Navy for Lume, navy for NPP
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className={`max-w-[85%] sm:max-w-sm p-0 border-[3px] shadow-2xl max-h-[88vh] overflow-y-auto`} style={{ borderColor: brandColor, backgroundColor: brandColor }}>
        <div className="m-1 border-2 rounded-md shadow-[inset_0_2px_6px_rgba(0,0,0,0.3)] bg-white p-3" style={{ borderColor: isLume ? "#0f1f33" : "#0f172a" }}>
