...
export function WeatherRadarModal({ open, onOpenChange, weather, location, onLocationChange }: WeatherRadarModalProps) {
  ...
  return (
    ...
    >
      <div 
        className="bg-gradient-to-br from-[#1a2e1a] to-[#0d1a0d] border border-white/20 rounded-xl w-[calc(100%-16px)] max-w-4xl overflow-hidden shadow-2xl m-2"
      ...
      >
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5 flex-wrap">
                <span className="text-lg font-light text-white">{weather?.temp || "--"}°F</span>
        ...
        {searchError && (
          <p className="text-red-400 text-[10px] px-3 py-1 bg-red-500/10">{searchError}</p>
        )}

        {/* Map - Fixed height for all screens */}
        <div className="relative h-40 sm:h-48 md:h-64 bg-black/20 overflow-hidden">
        ...
        <div className="px-2 py-1.5 bg-black/40 border-t border-white/10 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-0.5">
            <Button
              ...
              disabled={zoom <= 4}
              className="h-6 w-6 text-white hover:bg-white/10 disabled:opacity-30"
        ...
        <div className="p-2 border-t border-white/10">
          {weather ? (
            <div className="grid grid-cols-5 gap-1.5">
              ...
              <div className="bg-black/30 rounded p-1.5 text-center">
                <Sun className="w-3 h-3 text-amber-400 mx-auto mb-0.5" />
                <p className="text-[8px] text-gray-400">Feels</p>
                <p className="text-[11px] font-medium text-white">{weather.feelsLike}°</p>
              </div>
            </div>
          ) : (
