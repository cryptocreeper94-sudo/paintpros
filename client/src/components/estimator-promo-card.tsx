...
export function EstimatorPromoCard({ variant = 'default' }: EstimatorPromoCardProps) {
  ...
  return (
    <GlassCard 
      className={`relative overflow-hidden bg-gradient-to-br from-sky-500/10 via-teal-500/5 to-sky-500/10 border-sky-500/30 ${isCompact ? 'p-3' : 'p-4 md:p-6'}`}
      hoverEffect
      glow
    >
      <div className="absolute inset-0 bg-gradient-to-r from-sky-500/10 via-transparent to-teal-500/10 animate-pulse opacity-30" />
      
      <div className="relative z-10">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-2">
            <div className={`${isCompact ? 'p-2' : 'p-2.5'} rounded-xl bg-gradient-to-br from-sky-500 to-teal-500 shadow-lg shadow-sky-500/30`}>
          ...
          <Zap className={`${isCompact ? 'w-5 h-5' : 'w-6 h-6'} text-sky-500 animate-pulse`} />
        </div>
        ...
        <div className={`flex flex-wrap gap-2 mb-4 ${isCompact ? 'text-[9px]' : 'text-[10px] md:text-xs'}`}>
          <span className="px-2 py-1 rounded-full bg-sky-500/10 text-sky-600 dark:text-sky-400 border border-sky-500/20">
            Instant Quotes
          </span>
          <span className="px-2 py-1 rounded-full bg-sky-500/10 text-sky-600 dark:text-sky-400 border border-sky-500/20">
            AI-Powered
        ...
        <a href="/estimator-app" className="block" data-testid="link-estimator-promo">
          <Button 
            className="w-full bg-gradient-to-r from-sky-500 to-teal-500 text-white font-bold gap-2"
